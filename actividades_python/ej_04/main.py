...
R_REF = 10000.0  # Resistencia de referencia en ohms
...
def read_potentiometer():
    ...
    # Escalar el valor leído a un rango de 0 - 30 grados Celsius
...
def control_leds(temp_setpoint, actual_temp):
    # Determinar la variación entre la temperatura actual y la temperatura de la consigna
    difference = actual_temp - temp_setpoint
    
    # Establecer el límite máximo para ajustar la intensidad del LED según la variación de temperatura
    max_difference = 5.0
    
    # Determinar la intensidad luminosa del LED basándose en la variación de temperatura
    duty_cycle = min(abs(difference) / max_difference, 1.0)

    if difference > 0:
        # Si la temperatura actual es superior a la consigna, encender el LED azul
        blue_led.value = duty_cycle
        red_led.value = 0
    else:
        # Si la temperatura actual es inferior a la consigna, encender el LED rojo
